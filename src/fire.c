...
Int_Grid fire_grid = NULL; // Grid containing only fire. // Contains cells with wither FIRE_CELL or EMPTY_CELL values
...
/**
 * Determine all the risky, danger and non risky cells in the environment.
 * 
 * @note A risky cell is a cell adjacent to a fire and a wall (between both).
 * @note A danger cell is a cell adjacent to a fire that isn't a risky cell
 * @note A non risky cell is a cell too far away from a fire.
 */
// void determine_risky_cells()
// {
//     const int MUST_DANGER_CELL = -3000; // Indicates that a cell must be considered a danger cell, even if it can be classified as a Risky Cell in one of the requirements.
...
//             {
//                 risky_cells_grid[i][j] = FIRE_CELL;
//                 continue;
//             }
...
//         for(int j = 0; j < cli_args.global_column_number; j++)
//         {
//             // Find the cells with fire
//             if(fire_grid[i][j] != FIRE_CELL)
//                 continue;

//             for(int m = 0; m < 8; m++)
//             {
//                 int lin = i + moore_neighbor_modifiers[m].lin;
//                 int col = j + moore_neighbor_modifiers[m].col;

//                 if(! is_within_grid_lines(lin) || ! is_within_grid_columns(col) || 
//                     is_cell_with_fire((Location) {lin, col}) || obstacle_grid[lin][col] == IMPASSABLE_OBJECT)
//                     continue;

//                 // Cells with fire that aren't adjacent to a impassable object (including exits, in the obstacle_grid)
//                 if(adjacent_to_impassable_grid[i][j] == 0)
//                 {
//                     // If the given neighbor cell is next to a impassable object (exit or wall), then it is a Risky cell.
//                     if(adjacent_to_impassable_grid[lin][col] == 1 && risky_cells_grid[lin][col] != MUST_DANGER_CELL)
//                         risky_cells_grid[lin][col] = RISKY_CELL;
//                 }
//                 else // Cells with fire that are adjacent to a impassable object
//                 {
//                     if(moore_neighbor_modifiers[m].lin != 0 && moore_neighbor_modifiers[m].col != 0) // Diagonals
//                     {
//                         if(adjacent_to_impassable_grid[lin][col] == 1)
//                             risky_cells_grid[lin][col] = RISKY_CELL;

//                         continue;
//                     }

//                     // If the given neighbor cell is next to an exit, then it is a Risky Cell.
//                     if(adjacent_to_impassable_grid[lin][col] == 1 && exits_set.distance_to_exits_grid[lin][col] == 1.0)
//                         risky_cells_grid[lin][col] = RISKY_CELL;
//                     else // Else, it must be a danger cell
//                         risky_cells_grid[lin][col] = MUST_DANGER_CELL;
//                 }
//             }
//         }
//     }
...
//         for(int j = 0; j < cli_args.global_column_number; j++)
//         {
//             if(risky_cells_grid[i][j] == MUST_DANGER_CELL)
//                 risky_cells_grid[i][j] = DANGER_CELL;
//         }
//     }
// }
